...
const report = { report: 'PRODUCTION RESULT QUALITY', queriesTested: rows.length, distribution, averageResults: Math.round((allResults.length / rows.length) * 100) / 100, wrongTypeExposure, nonPremiumExposure, maxObserved: Math.max(...rows.map(({ results }) => results.length)), byQuery: rows.map(({ queryId, contentTypeId, results }) => ({ queryId, contentTypeId, count: results.length, ids: results.map(({ id }) => id), compositionFamilies: results.map(({ compositionFamily }) => compositionFamily) })) }
const lines = ['# PRODUCTION RESULT QUALITY', '', `Queries tested: ${report.queriesTested}`, `Average results: ${report.averageResults}`, `Maximum observed: ${report.maxObserved}`, `Wrong-type exposure: ${wrongTypeExposure}`, `Non-premium exposure: ${nonPremiumExposure}`, '', '## Result-count distribution', '', '| Count | Queries |', '| ---: | ---: |', ...Object.entries(distribution).map(([count, queries]) => `| ${count} | ${queries} |`), '']
