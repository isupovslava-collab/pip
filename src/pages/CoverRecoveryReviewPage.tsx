...
import { coverRecoveryRound2Candidates, coverRound1Decisions, coverRound2FinalDecisionLog, coverRound2FinalDecisions, type CoverRecoveryCandidate, type CoverRound2FinalDecision } from '../data/coverRecoveryCandidates'
...
export function CoverRecoveryReviewPage() {
  ...
  const exportReviews = () => {
    ...
    link.href = url
    link.download = 'cover-round-2-final-po-audit.json'
    link.click()
  ...
  return <section className="mx-auto max-w-7xl" aria-labelledby="cover-recovery-title">
    <header className="surface p-5 sm:p-8">
      <p className="eyebrow">Internal · Cover Round 2 Final</p>
      <h1 id="cover-recovery-title" className="mt-2 text-3xl font-bold text-navy sm:text-4xl">Cover Round 2 Final Decisions</h1>
      <p className="mt-3 max-w-4xl leading-7 text-muted">Финальные решения Product Owner зафиксированы {coverRound2FinalDecisionLog.reviewedAt}. В production curated core включены <strong className="text-navy">3 approved Cover references</strong>; revision-кандидат остаётся изолированным.</p>
      <div className="mt-5 flex flex-wrap items-center gap-3"><button type="button" onClick={exportReviews} className="btn-primary">Export Final PO Audit JSON</button><span className="text-sm font-semibold text-muted">Выбрано для сравнения: <strong className="text-navy">{compareIds.length} / 3</strong></span></div>
    ...
    <section className="surface mt-6 p-5 sm:p-6" aria-labelledby="cover-final-ranking-title">
      <h2 id="cover-final-ranking-title" className="text-2xl font-bold text-navy">Final PO ranking</h2>
      <div className="mt-5 grid gap-4 lg:grid-cols-2">{coverRound2FinalDecisions.map((decision) => <article key={decision.candidateId} className="rounded-2xl border border-line bg-white p-4"><div className="flex flex-wrap items-center gap-2"><Badge>{`#${decision.rank}`}</Badge><DecisionBadge decision={decision.decision} /><Badge>{decision.priority}</Badge></div><h3 className="mt-3 text-lg font-bold text-navy">{decision.title}</h3><p className="mt-2 text-sm leading-6 text-muted">{decision.notes}</p><p className="mt-3 text-xs font-bold uppercase tracking-wide text-blue">{decision.productionReferenceId ? `Production: ${decision.productionReferenceId}` : 'Production exposure: 0'}</p></article>)}</div>
    </section>
...
function CandidateCard({ candidate, finalDecision, review, compared, compareDisabled, onCompare, onChange }: { candidate: CoverRecoveryCandidate; finalDecision?: CoverRound2FinalDecision; review: CoverRecoveryReview; compared: boolean; compareDisabled: boolean; onCompare: () => void; onChange: (change: Partial<CoverRecoveryReview>) => void }) {
  return <article className="surface overflow-hidden">
    ...
    <div className="p-5 sm:p-7">
      <div className="flex flex-wrap items-center gap-2"><Badge>{candidate.id}</Badge><Badge>{candidate.visualFamily}</Badge><Badge>{candidate.origin}</Badge>{finalDecision ? <DecisionBadge decision={finalDecision.decision} /> : <Badge>NOT SELECTED</Badge>}<label className="ml-auto inline-flex min-h-11 items-center gap-2 rounded-xl border border-line px-3 text-sm font-semibold text-navy"><input type="checkbox" aria-label={`Сравнить ${candidate.id}`} checked={compared} disabled={compareDisabled} title={compareDisabled ? 'Можно сравнить максимум 3 варианта' : undefined} onChange={onCompare} />Compare</label></div>
      <h2 className="mt-4 text-2xl font-bold text-navy sm:text-3xl">{candidate.title}</h2>
      <dl className="mt-5 grid gap-4 text-sm sm:grid-cols-2 lg:grid-cols-4"><Meta label="Visual family" value={candidate.visualFamily} /><Meta label="Origin" value={candidate.origin} /><Meta label="Parent candidate" value={candidate.parentCandidateId ?? '—'} /><Meta label="Production exposure" value={finalDecision?.decision === 'approved' ? `true · ${finalDecision.productionReferenceId}` : 'false'} /><Meta label="Current → proposed type" value={`${candidate.currentContentType} → ${candidate.proposedContentType}`} /><Meta label="Composition family" value={candidate.compositionFamily} /><Meta label="Resolution" value={`${candidate.width}×${candidate.height}`} /><Meta label="Known overlap issue" value={candidate.knownOverlapIssue ? 'yes · revision required' : 'no'} /></dl>
      <div className="mt-5 grid gap-4 lg:grid-cols-2"><Info title="Почему это может быть сильным титульным слайдом" text={candidate.rationale} /><Info title={finalDecision ? `Final PO: ${finalDecision.decision.toUpperCase()}` : 'Final PO status'} text={finalDecision?.notes ?? 'В финальный рейтинг не вошёл; production exposure отсутствует.'} /></div>
...
function DecisionBadge({ decision }: { decision: CoverRound2FinalDecision['decision'] }) { return <span className={`rounded-full px-3 py-1 text-xs font-bold ${decision === 'approved' ? 'bg-emerald-100 text-emerald-900' : 'bg-amber-100 text-amber-900'}`}>{decision === 'approved' ? 'APPROVE' : 'REVISE'}</span> }
