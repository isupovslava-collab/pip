...
describe('SearchWizard', () => {
  ...
  it('проводит пользователя через пять шагов и запускает рекомендации', async () => {
    ...
    expect(screen.getByText('Вы уже выбрали:')).toBeInTheDocument()
    expect(screen.getByText('Защита бюджета')).toBeInTheDocument()
    ...
    expect(screen.getByText('Сейчас выберите тип слайда')).toBeInTheDocument()
    expect(screen.getByText('Руководитель компании')).toBeInTheDocument()
    await user.click(screen.getByRole('button', { name: 'Изменить' }))
    expect(screen.getByLabelText(/Защита бюджета/)).toBeChecked()
    await user.click(screen.getByRole('button', { name: 'Далее' }))
    await user.click(screen.getByRole('button', { name: 'Далее' }))
    await user.click(screen.getByRole('button', { name: 'Далее' }))
    await user.click(screen.getByRole('button', { name: 'Далее' }))
