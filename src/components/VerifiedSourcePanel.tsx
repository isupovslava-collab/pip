...
export function VerifiedSourcePanel({ sources }: { sources: SourceReference[] }) {
  ...
  const eligibleSources = sources.filter(isProductionEligibleSourceReference)
  if (!eligibleSources.length) return null
        ...
        return <article key={source.id} className={index ? 'border-t border-line pt-5' : ''}>
          <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2"><span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-bold uppercase tracking-wide text-emerald-800">Проверенный источник</span><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-muted">PIP Approved</span></div>
              <h2 id={index === 0 ? 'verified-source-heading' : undefined} className="mt-3 text-xl font-semibold text-navy">Проверенный первоисточник</h2>
            ...
            <a href={href} target="_blank" rel="noreferrer" onClick={() => feedback.logEvent('verified_source_opened', source.id)} className="btn-primary shrink-0">Открыть первоисточник<Icon name="arrow-right" className="h-4 w-4" /></a>
          </div>
