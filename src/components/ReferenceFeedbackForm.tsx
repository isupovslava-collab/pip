...
export function ReferenceFeedbackForm({ referenceId }: { referenceId: string }) {
  ...
  return <section className="surface mt-6 p-5 sm:p-7" aria-labelledby="reference-feedback-title">
    ...
    {useful !== null && <><label className="mt-5 block text-sm font-semibold text-navy">Комментарий (необязательно)<textarea value={comment} onChange={(event) => { setComment(event.target.value); setSaved(false) }} rows={3} maxLength={FEEDBACK_COMMENT_MAX_LENGTH} className="mt-2 w-full resize-y rounded-xl border border-line px-4 py-3 font-normal" /></label><div className="mt-2 flex justify-between gap-3 text-xs text-muted"><p>Не указывайте персональные данные.</p><span aria-live="polite" className="shrink-0 tabular-nums">{comment.length} / {FEEDBACK_COMMENT_MAX_LENGTH}</span></div><div className="mt-4 flex items-center gap-4"><button type="button" className="btn-primary" onClick={() => { submitReferenceFeedback({ referenceId, useful, issues: useful ? [] : issues, comment: comment.trim() }); setSaved(true) }}>Сохранить</button>{saved && <span role="status" className="text-sm font-semibold text-success">Отзыв сохранён</span>}</div></>}
  </section>
}
